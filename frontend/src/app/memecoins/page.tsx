...
export default function MemeCoinsPage() {
  const { data, loading, refetch } = useApi(() => memecoinsApi.rankings(), []);
  ...
  return (
    <div>
      <TopBar title="Meme Coins" subtitle={`${data?.data.length ?? 0} ranked`} meta={data?.meta} />
      <div className="p-6">
        <MemeCoinTable coins={data?.data ?? []} loading={loading} onWatchlist={handleWatchlist} />
      </div>
    </div>
  );
}
