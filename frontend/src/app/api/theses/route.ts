...
export async function GET(req: NextRequest) {
  try {
    ...
    if (ticker) return NextResponse.json(await sb(`ai_theses?select=*&ticker=eq.${ticker.toUpperCase()}&order=created_at.desc`));
    return NextResponse.json(await sb('ai_theses?select=*&order=created_at.desc'));
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = { ...body, ticker: String(body.ticker || '').toUpperCase(), catalyst_id: body.catalyst_id || null, source_ids: body.source_ids || [], source_summary: body.source_summary || null, user_id: body.user_id || null };
